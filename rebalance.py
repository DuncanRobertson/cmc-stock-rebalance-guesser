...
def read_cmc_pnl_to_portfdict(filename,desiredport):
   ...
   for row in pnlcsv:
      if row[5] != 'Market Value $':
         # in case share price is blank, like in TOTALs row
         if row[1] == '':
            row[1] = '0.0'
	 currentport[row[0]] = float(row[5]),float(row[1])
   ...
   for asxcode in currentport.keys():
	 ...
	 currentport[asxcode] = currentport[asxcode][0],currentport[asxcode][0]/currentport[TOTALS][0] * 100,currentport[asxcode][1]
      else:
	 currentport[asxcode] = currentport[asxcode][0],currentport[asxcode][0]/currentport[TOTALS][0] * 100,currentport[asxcode][1]
   ...
   # containing tuple: desired percentage,current value dollars,current percentage dollars, share price with shares we dont have yet set to zero.
   for asxcode in desiredport.keys():
      if currentport.has_key(asxcode):
	 desiredport[asxcode] = desiredport[asxcode],currentport[asxcode][0],currentport[asxcode][1],currentport[asxcode][2]
      else:
	 desiredport[asxcode] = desiredport[asxcode],0.0,0.0,0.0
...
def rebalance(amount,portpassed,codetobuy):
   ...
   for asxcode in portf.keys():
      ...
      if asxcode == TOTALS:
         portf[asxcode] = portf[asxcode][0],newtotal,portf[asxcode][2],portf[asxcode][3]
   return portf
...
def printport(portf):
   ...
   print "%8s%12s%12s%12s%12s" % ('ASXcode','%wanted','$total','%current','share price')
   for asxcode in asxcodes:
      print "%8s%12.2f%12.2f%12.2f%12.2f" % ((asxcode,) + portf[asxcode])
